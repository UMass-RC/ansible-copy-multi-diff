                    ...
                    if local_follow:
                        if (dir_stats.st_dev, dir_stats.st_ino) in parent_dirs:
                            ...
                            for parent in range(len(parent_dir_list), 0, -1):
                                parent_stat = os.stat("/".join(parent_dir_list[:parent]))
...
class ActionModule(ActionBase):
    ...
    def _copy_file(
        ...
        if local_checksum != dest_status["checksum"]:
            ...
            new_module_args.update(
                dict(src=tmp_src, dest=dest, _original_basename=source_rel, follow=follow)
            )
            ...
            module_return = self._execute_module(
                module_name="ansible.legacy.copy", module_args=new_module_args, task_vars=task_vars
            )
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        # look up the files and use the first one we find as src
            ...
            result.update(
                self._execute_module(module_name="ansible.legacy.copy", task_vars=task_vars)
            )
            return self._ensure_invocation(result)
        else:
