...
def copy_common_dirs(src, dest, module):
    ...
    for item in common_dirs:
        ...
        changed = (
            copy_common_dirs(os.path.join(src, item), os.path.join(dest, item), module) or changed
        )
    return changed
    ...
    if dest.endswith(os.sep):
        ...
        if not os.path.exists(b_dirname):
            ...
            if module.check_mode:
                module.exit_json(
                    msg="dest directory %s would be created" % dirname, changed=True, src=src
                )
        ...
        if not os.path.exists(os.path.dirname(b_dest)):
            try:
                ...
                if "permission denied" in to_native(e).lower():
                    module.fail_json(
                        msg="Destination directory %s is not accessible" % (os.path.dirname(dest))
                    )
            module.fail_json(
        ...
        if not module.check_mode:
            try:
                ...
                if validate:
                    ...
                    if rc != 0:
                        module.fail_json(
                            msg="failed to validate", exit_status=rc, stdout=out, stderr=err
                        )
                ...
                module.fail_json(
                    msg="failed to copy: %s to %s" % (src, dest), traceback=traceback.format_exc()
                )
        changed = True
